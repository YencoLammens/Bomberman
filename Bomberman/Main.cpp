...
void load()
{
	...
	dae::Camera::GetInstance().SetScreenSize(960.f, 615.f);
	dae::Camera::GetInstance().SetMapSize(1920.f, 1230.f);
	...
	dae::ResourceManager::GetInstance().LoadTexture("Bombermap.png");
	...
	renderComponent->SetTexture(dae::ResourceManager::GetInstance().LoadTexture("Bombermap.png"));
	...
	//Bind keyboard inputs to BOMBERMAN
	//Pressed
	...
	//HUD displays for bOMBERMAN

	//Health display BOMBERMANNNN
	auto hpGo = std::make_unique<dae::GameObject>();
	//hpGo->GetTransform()->SetLocalPosition(500.f, 10.f, 0.f); // initial fallback
	auto hpText = std::make_unique<dae::TextComponent>(hpGo.get(), "LEFT 2", font.get());
	hpGo->AddComponent(std::move(hpText));
	auto hpDisplay = std::make_unique<dae::HPDisplay>(hpGo.get());
	hpGo->AddComponent(std::move(hpDisplay));
	go->AddObserver(hpGo->GetComponent<dae::HPDisplay>());
	scene.Add(std::move(hpGo));
	...
	//Score display bomberman
	auto scoreGo = std::make_unique<dae::GameObject>();
	//scoreGo->GetTransform()->SetLocalPosition(500, 10, 0);
	std::string scoreString = std::format("{:02}", go->GetComponent<dae::ScoreComponent>()->GetScore());
